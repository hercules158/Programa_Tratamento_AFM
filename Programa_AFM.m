...
while txtIndex <= txtSize
    ...
    NumYaxis = Offset(NumYaxis); %Função que aplica o offset
        ...
        if NumXaxis(1) > NumXaxis(length(NumXaxis)) %Teste para saber se é o gráfico de volta
    ...
    SaveFile(identationMatrix, txtIndex, save_name, save_directory, txtSize, K_Force_matrix); %Função que salva
